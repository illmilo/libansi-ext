...
// --preload-file JuliaMono-Regular.ttf@JuliaMono-Regular.ttf
// -lembind
// -sUSE_FREETYPE=1
// -sALLOW_MEMORY_GROWTH
// -sPTHREAD_POOL_SIZE=navigator.hardwareConcurrency
//
// IMPORTANT: Make sure to put some font in the ./ directory and replace
...
std::string EmLoadDefaultTTF(AnsiArt &art) {
  std::string err;
  std::string ttf = ReadFile("JuliaMono-Regular.ttf", err);
  if (!err.empty()) {
    return err;
  }
